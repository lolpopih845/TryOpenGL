...
namespace Game {
    ...
    T* LoadAsset(Args&&... args) { return Asset::AssetStorage<T>::Load(std::forward<Args>(args)...);}
    template <typename T>
    T* GetAsset(const std::string& id) { return Asset::AssetStorage<T>::Get(id); }

    inline auto& OBJ() {
        return Engine::Scene::GetCurrentScene()->objects;
    }
    ...
    inline auto& CAM() {
        return Engine::Scene::GetCurrentScene()->cameras;
    };
    ...
    inline auto& STAGE() {
        return Engine::Scene::GetCurrentScene()->lighting;
    }
